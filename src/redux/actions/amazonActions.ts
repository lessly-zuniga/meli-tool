...
import { AmazonAction, AmazonActionTypes } from '../types/amazonTypes';
  ...
  return async (dispatch: Dispatch<AmazonAction>) => {
    try {
      ...
      if (Array.isArray(response) && response.length > 0) {
        ...
        dispatch({ type: AmazonActionTypes.SET_AMAZON_PRODUCTS, payload: response });
      } else {
        // Dispatch de la acción de error si la respuesta no es válida
        dispatch({ type: AmazonActionTypes.SET_AMAZON_ERROR, payload: 'La respuesta de Amazon no contiene datos válidos.' });
      }
      return response;
    } catch (error: any) {
      // Dispatch de la acción de error
      dispatch({ type: AmazonActionTypes.SET_AMAZON_ERROR, payload: error.message });
    }
  };
};
